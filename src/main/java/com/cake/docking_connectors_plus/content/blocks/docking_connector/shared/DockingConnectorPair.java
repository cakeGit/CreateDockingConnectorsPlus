...
public class DockingConnectorPair<T extends BlockEntity & DockingConnector> extends MagnetPair<T> {
    ...
    public static Vector3d getRelativeTipPosition(final DockingConnector magnet1, final DockingConnector magnet2, final Vector3d dest) {
    ...
    public static Vector3d getAverageTipPosition(final DockingConnector magnet1, final DockingConnector magnet2, final Vector3d dest) {
        final Vec3 plotPos1 = magnet1.getTipPosition();
        final Vec3 plotPos2 = magnet2.getTipPosition();
    ...
    private void toggleLock(final boolean newLock, final DockingConnector dock1, final DockingConnector dock2) {
    ...
    public void dock(final boolean force) {
        if (this.level.getBlockEntity(this.blockPos1) instanceof final DockingConnector dock1 &&
                this.level.getBlockEntity(this.blockPos2) instanceof final DockingConnector dock2 &&
                dock1.isExtended() &&
                dock2.isExtended()) {
            ...
            final double minDist = Math.min(dock1.getClosestPairDistance(), dock2.getClosestPairDistance());
            ...
            if (force || (t.lengthSquared() < angleTolerance * angleTolerance && r.lengthSquared() < distanceTolerance * distanceTolerance)) {
                if (force) {
                    locked = true;
                    dock1.setDockingState(DockingConnectorState.LOCKING);
                    dock2.setDockingState(DockingConnectorState.LOCKING);
                } else {
                    final boolean previousLockingState = dock1.getDockingState() == DockingConnectorState.LOCKED && dock2.getDockingState() == DockingConnectorState.LOCKED;
                ...
                if (!this.level.isClientSide()) {
                    final ServerSubLevel firstSubLevel = (ServerSubLevel) dock1.getLatestSubLevel();
                    final ServerSubLevel secondSubLevel = (ServerSubLevel) dock2.getLatestSubLevel();
                ...
                if (dock1.getDockingState() == DockingConnectorState.LOCKING && dock2.getDockingState() == DockingConnectorState.LOCKING) {
                    dock1.unDock();
                    dock2.unDock();
                }
            }
    ...
    public void unDock() {
        ...
        if (blockEntity1 instanceof final DockingConnector dock1 && blockEntity2 instanceof final DockingConnector dock2) {
            dock1.unDock();
            dock2.unDock();
    ...
    protected double preventPairFightingScale(final double distance, final T magnet1, final T magnet2) {
        final double d = Math.min(magnet1.getClosestPairDistance(), magnet2.getClosestPairDistance());
        if (d < distance * 0.9) {
